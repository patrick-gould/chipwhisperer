...
We don't really care about the computations being correct, only that they happen. So, we cap things off here
to avoid having to implement the whole bootloader.
Above, there is a lot of code that got commented out since it was not being used by the main program.
*/
