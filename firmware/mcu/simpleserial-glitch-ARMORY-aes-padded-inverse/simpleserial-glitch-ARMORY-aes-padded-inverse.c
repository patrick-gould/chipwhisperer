...
// Prototypes
void AES_ECB_encrypt(const u8 *, u8 *);
...
{
    // Set twice to avoid skipping assignment.
...
#endif
{
    ...
    asm("NOP");             // Pad trigger from glitch
    asm("NOP");             // Pad trigger from glitch
    asm("NOP");             // Pad trigger from glitch
    asm("NOP");             // Pad trigger from glitch
    asm("NOP");             // Pad trigger from glitch
...
int __attribute__((noinline)) testRunner(uint8_t zero){
    ...
    asm("addi a5, x0, 0");  // Flush garbage in a5
    ...
    // Call super_secret_function() twice to avoid skipping call.
    ...
    asm("NOP");             // Pad return from glitch
    asm("NOP");             // Pad return from glitch
    asm("NOP");             // Pad return from glitch
    asm("NOP");             // Pad return from glitch
    asm("NOP");             // Pad return from glitch
    ...
    return retVal; // Skipping this will cause a crash.
}
...
void AES_ECB_encrypt(const u8 master_key[16], u8 buf[16])
{
    ...
    //report_done(); Ignore this. ARMORY added this for their system, it does not help us.
    return;
}
